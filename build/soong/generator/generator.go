...
func init() {
	android.RegisterModuleType("starlight_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = starlightExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = starlightExpandVariables(ctx, dep_file)
	...
	cmd := starlightExpandVariables(ctx, String(g.properties.Cmd))
